...
class CDTrainer():
    ...
    def _forward_pass(self, batch):
        ...
        self.G_pred, self.off_pred, self.A_ligned_pred = self.net_G(img_in1, img_in2)
    ...
    def _backward_G(self):
        ...
        off_gt = self.batch['OFF_L'].to(self.device)
        img_a_src = self.batch['S_A'].to(self.device)
        # 
        ...
        if self.multi_scale_train == "True":
            ...
            if self.apt:
                mse = nn.MSELoss()
                l1 = nn.L1Loss()
                off_loss = mse(self.off_pred, off_gt)
                apt_loss = l1(self.A_ligned_pred, img_a_src)
                # print(self.G_loss, off_loss, apt_loss)
                self.G_loss = self.G_loss + off_loss*0.001 + apt_loss*0.5
        else:
